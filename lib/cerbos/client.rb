...
module Cerbos
  ...
  class Client
    ...
    #
    # @param target [String] Cerbos PDP server address (`"host:port"` or `"unix:/path/to/socket"`).
    ...
    #
    # @example Connect via TCP with TLS, verifying the server certificate using public root certificate authorities
    #   client = Cerbos::Client.new("cerbos.example.com:443", tls: Cerbos::TLS.new)
    def initialize(target, tls:, grpc_channel_args: {}, timeout: nil)
      handle_errors do
